...
BASE_JAR_PATH="Runner.Runner"
DIFF_JAR_PATH="${SCRIPT_DIR}/build/libs/ARDiff-test-1.0-SNAPSHOT-all.jar"
...
for run_settings in "${runs_settings[@]}"; do
  ...
  case $tool in
      "ARDiff-base")
          ...
          command="java -jar target/ardiff-test-1.0-SNAPSHOT.jar  Runner.Runner --path1 ${oldV} --path2 ${newV} --tool A --s coral --H H123 --b ${depth_limit}" #> ${directory}/ResultArDiffThres_Var_115.txt"
          ;;
      "Impacted-base")
