...
for run_settings in "${runs_settings[@]}"; do
  ...
  case $tool in
      "ARDiff-base")
          command="timeout --verbose --foreground ${timeout}s java -jar '${BASE_JAR_PATH}' --path1 ${oldV} --path2 ${newV} --tool A --s coral --H H123 --b ${depth_limit} --t ${timeout} > ${directory}/ResultArDiff.txt"
          ;;
      "Impacted-base")
          command="timeout --verbose --foreground ${timeout}s java -jar '${BASE_JAR_PATH}' --path1 ${oldV} --path2 ${newV} --tool I --s coral --b ${depth_limit} --t ${timeout} > ${directory}/ResultImpacted.txt"
          ;;
      "DSE-base")
          command="timeout --verbose --foreground ${timeout}s java -jar '${BASE_JAR_PATH}' --path1 ${oldV} --path2 ${newV} --tool D --s coral --b ${depth_limit} --t ${timeout} > ${directory}/ResultDSE.txt"
          ;;
      "PASDA-base")
          command="timeout --verbose --foreground ${timeout}s java -jar '${BASE_JAR_PATH}' --path1 ${oldV} --path2 ${newV} --tool P --s coral --b ${depth_limit} --t ${timeout} > ${directory}/ResultPASDAbase.txt"
          ;;
      "PASDA-diff")
          command="timeout --verbose --foreground ${timeout}s java -jar '${DIFF_JAR_PATH}' ${directory} PASDA ${timeout} ${depth_limit} > ${directory}/ResultPASDA.txt"
          ;;
