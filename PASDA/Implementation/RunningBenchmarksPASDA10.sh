...
for run_settings in "${runs_settings[@]}"; do
  ...
  case $tool in
      "PASDA-base")
          command="timeout --verbose --foreground ${timeout}s java -jar '${BASE_JAR_PATH}' --path1 ${oldV} --path2 ${newV} --tool P --s coral --b ${depth_limit} --t ${timeout} > ${directory}/ResultPASDAD10_120base.txt"
          ;;
      "PASDA-diff")
          command="timeout --verbose --foreground ${timeout}s java -jar '${DIFF_JAR_PATH}' ${directory} PASDA ${timeout} ${depth_limit} > ${directory}/ResultPASDAD10_120.txt"
          ;;
      *)
