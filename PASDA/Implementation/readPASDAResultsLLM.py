...
worksheet.write("T1", "Total False Positive")
totalEq=0
totalNeq=0
totalUnknown=0
totalMaybeEq=0
...
for folder in folders:
    ...
    for subfolder in subfolders:
        if whichFolder=="Eq" and subfolder in ignoreEq:
            continue
        if whichFolder=="NEq" and subfolder in ignoreNeq:
            continue
        ...
        for file in subsubfolders:
            ...
            # for file in subsubsubfolders:
                if file==fileName+".txt":
                    ...
                    lines=open(subsubfolderName+file).readlines()
                    pasdaBaseLines=open(subsubfolderName+fileName+"Base.txt").readlines()
                    ...
                    # print(time)
                    # print("=====================================")
                    worksheet.write("A"+str(row), subsubfolderName.split(name+"/")[-1])
                    ...
                    if subsubfolderName.split("/")[-2]=="NEq" and result=="EQ":
                        mismatchCount+=1
                        print(subsubfolderName)
                    if subsubfolderName.split("/")[-2]=="Eq" and result=="NEQ":
                        mismatchCount+=1
                        print(subsubfolderName)
                    if "/Eq" in subsubfolderName and result=="MAYBE_NEQ":
                        falsePositive+=1
                        print(subsubfolderName, result)
                    if "NEq" in subsubfolderName and result=="MAYBE_EQ":
                        falsePositive+=1
                        print(subsubfolderName, result)
