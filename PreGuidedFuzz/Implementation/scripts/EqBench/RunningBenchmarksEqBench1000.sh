...
for run_settings in "${runs_settings[@]}"; do
  ...
  case $tool in
      ...
      "PASDA-base")
          ...
          ;;
      "PASDA-diff")
          command="timeout --verbose --foreground ${timeout}s java -jar '${DIFF_JAR_PATH}' ${directory} PASDA ${timeout} ${depth_limit} ${fuzztime} ${fuzzType} > ${directory}/ResultBFuzz1000.txt"
          ;;
      "SE-base")
